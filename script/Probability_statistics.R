...
if (!file.exists(CLEAN_FILE)) {
  stop("Cleaned data not found. Please run Cleaning_Data.R first.")
}
...
print_table <- function(df, title = NULL, max_rows = 30) {
  ...
  for (i in seq_along(headers)) {
    cat(" ", sprintf(paste0("%-", widths[i], "s"), trim_cell(headers[i], widths[i])), " |", sep = "")
  }
  cat("\n")
  print_line(widths)
  ...
  for (r in seq_len(nrow(df))) {
    cat("|")
    for (c in seq_along(headers)) {
      cat(" ", sprintf(paste0("%-", widths[c], "s"), trim_cell(df[r, c], widths[c])), " |", sep = "")
    }
    cat("\n")
  }
...
format_numbers_df <- function(df) {
  ...
  for (col in names(out)) {
    if (is.numeric(out[[col]])) out[[col]] <- format_num(out[[col]])
  }
  out
}
...
group_numeric_stats <- function(data, group_col, value_col) {
  ...
  result <- do.call(rbind, lapply(names(groups), function(g) {
    ...
    data.frame(
      ...
      nho_nhat = min(x),
      lon_nhat = max(x),
      phuong_sai = safe_var(x),
      do_lech_chuan = safe_sd(x),
...
probability_table <- function(data, group_col, event_col) {
  ...
  totals <- aggregate(
    so_luong ~ key,
    data = data.frame(key = tab[[group_col]], so_luong = tab$so_luong),
    FUN = sum
  )
...
overview <- data.frame(
  ...
  gia_tri = c(
    ...
    length(unique(data_clean$source)),
...
basic_prob <- data.frame(
  ...
  so_luong = c(
    sum(data_clean$transmission == "Automatic", na.rm = TRUE),
    sum(data_clean$transmission == "Manual", na.rm = TRUE),
    sum(data_clean$transmission == "CVT", na.rm = TRUE),
    sum(data_clean$is_high_price, na.rm = TRUE),
    sum(data_clean$is_high_mileage, na.rm = TRUE),
    sum(data_clean$age_group == "0-3 nam", na.rm = TRUE),
    sum(data_clean$age_group == "Tren 12 nam", na.rm = TRUE)
  )
)
...
prob_high_price_by_brand <- prob_high_price_by_brand[
  as.character(prob_high_price_by_brand$is_high_price) == "TRUE",
]
prob_high_price_by_brand <- prob_high_price_by_brand[
  order(-prob_high_price_by_brand$xac_suat, -prob_high_price_by_brand$so_luong),
]
...
bayes_auto_new$xac_suat <- ifelse(
  bayes_auto_new$tong_xe_automatic == 0,
  NA,
  bayes_auto_new$so_xe_automatic_va_moi / bayes_auto_new$tong_xe_automatic
)
...
if (length(unique(data_ttest$transmission)) == 2 && all(table(data_ttest$transmission) >= 2)) {
  ...
  test_results <- rbind(test_results, data.frame(
    kiem_dinh = "T-test: price ~ transmission",
    thong_ke = paste0("t = ", round(ttest$statistic, 4)),
    p_value = ttest$p.value,
    ket_luan = ifelse(ttest$p.value < 0.05, "Co khac biet gia trung binh", "Chua du bang chung khac biet"),
    stringsAsFactors = FALSE
  ))
}
...
if (all(dim(chisq_table) > 1)) {
  ...
  test_results <- rbind(test_results, data.frame(
    kiem_dinh = "Chi-square: transmission vs age_group",
    thong_ke = paste0("X2 = ", round(chisq$statistic, 4)),
    p_value = chisq$p.value,
    ket_luan = ifelse(chisq$p.value < 0.05, "Co moi lien he thong ke", "Chua du bang chung co moi lien he"),
    stringsAsFactors = FALSE
  ))
}
...
if (length(unique(data_clean$price)) > 1 && length(unique(data_clean$mileage)) > 1) {
  ...
  test_results <- rbind(test_results, data.frame(
    kiem_dinh = "Pearson correlation: price vs mileage",
    thong_ke = paste0("r = ", round(cor_pm$estimate, 4)),
    p_value = cor_pm$p.value,
    ket_luan = ifelse(cor_pm$p.value < 0.05, "Co tuong quan thong ke", "Chua du bang chung co tuong quan"),
    stringsAsFactors = FALSE
  ))
}
...
report_lines <- capture.output({
  cat("BAO CAO XAC SUAT - THONG KE MO TA\n")
  cat("Ngay chay: ", as.character(Sys.Date()), "\n", sep = "")
  cat("File du lieu da lam sach: ", CLEAN_FILE, "\n", sep = "")
