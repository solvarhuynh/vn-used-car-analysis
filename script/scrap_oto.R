# Scrape Oto.com.vn used-car listings by HTML parsing.
# Output: data/data_oto_raw.csv with the canonical 18-column schema.
...
SCRIPT_NAME <- "scrap_oto.R"
OUTPUT_FILE <- "data/data_oto_raw.csv"
LISTING_URL <- "https://oto.com.vn/mua-ban-xe"
SOURCE_NAME <- "oto.com.vn"
DISPLAY_NAME <- "Oto.com.vn"

absolute_url <- function(path) {
  ifelse(str_detect(path, "^https?://"), path, paste0("https://oto.com.vn", path))
}
...
scrape_detail_page <- function(url) {
  ...
  # Selectors are starter selectors for boilerplate. Refine them against the
  # current Oto.com.vn markup before production use.
  tibble(
...
scrape_oto <- function(max_pages = 1) {
  cat(sprintf("\nStarting %s HTML scraping...\n", DISPLAY_NAME))
  log_message(SCRIPT_NAME, "Starting Oto.com.vn HTML scrape.")
  ...
  tryCatch({
    ...
    for (page_num in seq_len(max_pages)) {
      page_url <- if (page_num == 1) LISTING_URL else paste0(LISTING_URL, "/p", page_num)
      ...
      links <- listing_page %>%
        ...
        na.omit() %>%
        unique() %>%
        keep(~ str_detect(.x, "mua-ban-xe|/xe-")) %>%
        absolute_url()
    ...
    log_message(SCRIPT_NAME, sprintf("Finished Oto.com.vn scrape with %s records.", nrow(df)))
